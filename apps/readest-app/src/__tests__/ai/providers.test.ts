...
describe('GeminiProvider', () => {
  test('should throw if no API key', () => {
    const settings: AISettings = { ...DEFAULT_AI_SETTINGS, enabled: true, provider: 'google-gemini' };

    expect(() => new GeminiProvider(settings)).toThrow('API key required');
  });
  ...
  test('healthCheck should return false if key does not exist', async () => {
    ...
    (provider as unknown as { settings: AISettings }).settings.geminiApiKey = '';
    const result = await provider.healthCheck();
    expect(result).toBe(false);
  });
});
...
describe('getAIProvider', () => {
  ...
  test('should return GeminiProvider for google-gemini', () => {
    const settings: AISettings = {
      ...DEFAULT_AI_SETTINGS,
      enabled: true,
      provider: 'google-gemini',
