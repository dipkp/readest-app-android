...
function chatModelIdFor(settings: AISettings): string {
  switch (settings.provider) {
    ...
    case 'google-gemini':
      return (
        settings.geminiCustomModel || settings.geminiModel || 'gemini-2.5-flash-lite'
      );
    case 'groq':
      return (
        settings.groqCustomModel || settings.groqModel || 'llama-3.1-8b-instant'
      );
    case 'openrouter':
...
function embeddingModelIdFor(settings: AISettings): string {
  switch (settings.provider) {
    ...
    case 'groq':
      return settings.groqEmbeddingModel || 'nomic-embed-text';
    case 'openrouter':
