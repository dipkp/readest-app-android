...
import { resolveGoogleAccountLabel, resolveGoogleAccountProfile, type GoogleAccountProfile } from './googleDriveAuth';
...
export class WebDriveAuth implements DriveAuth {
  ...
  async accountProfile(): Promise<GoogleAccountProfile | null> {
    const token = await this.getAccessToken();
    return resolveGoogleAccountProfile(token, this.fetchFn);
  }
}
