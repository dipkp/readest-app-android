...
import { MdCloudSync, MdSync } from 'react-icons/md';
...
const SettingsMenu: React.FC<SettingsMenuProps> = ({ onPullLibrary, setIsDropdownOpen }) => {
  ...
  const avatarUrl = user?.avatar_url || user?.['user_metadata']?.['picture'] || user?.['user_metadata']?.['avatar_url'];
  ...
  // cursors freeze while DOD Cloud is off (the book/progress/note channels
  ...
  return (
    ...
    >
      {user ? (
        ...
        >
          <ul className='ms-0 flex flex-col ps-0 before:hidden'>
            ...
            />
            <MenuItem label={_('Sign Out')} onClick={() => { logout(); setIsDropdownOpen?.(false); }} />
          </ul>
        </MenuItem>
      ) : (
        <MenuItem label={_('Sign in with Google')} Icon={PiUserCircle} onClick={() => { login(); setIsDropdownOpen?.(false); }} />
      )}
        ...
        <ul className='ms-0 flex flex-col ps-0 before:hidden'>
          ...
          {!isPinEnabled && (
            <MenuItem
              ...
              tooltip={
                appService?.isMobileApp
                  ? _('Require a PIN (and biometrics, if available) to open DOD')
                  : _('Require a 4-digit PIN to open DOD')
              }
      ...
      <MenuItem label={_('About DOD')} onClick={showAboutReadest} />
    </Menu>
  );
};
