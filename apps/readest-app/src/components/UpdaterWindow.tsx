  ...
  useEffect(() => {
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `DOD_${data.version}_${arch}-portable.exe`;
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `DOD_${data.version}_${arch}.AppImage`;
    ...
    const buildNightlyUpdate = (n: ResolvedNightlyUpdate): GenericUpdate => ({
      ...
      downloadAndInstall: async (onEvent) => {
        ...
        const fileName = n.url.split('/').pop() || `DOD_${n.version}`;
        let filePath: string;
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of DOD is available!')}
              </h2>
              <p className='mb-2'>
                {_('DOD {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in DOD")}
