...
import { DEFAULT_AI_SETTINGS, GEMINI_MODELS, GROQ_MODELS, MODEL_PRICING } from '@/services/ai/constants';
...
const getModelOptions = (): ModelOption[] => [
  {
    ...
    inputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_FLASH]?.input ?? '?',
    outputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_FLASH]?.output ?? '?',
  },
  {
    id: GEMINI_MODELS.GEMINI_2_5_PRO,
    label: 'Gemini 2.5 Pro',
    inputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_PRO]?.input ?? '?',
    outputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_PRO]?.output ?? '?',
  },
  {
    id: GEMINI_MODELS.GEMINI_2_5_FLASH_LITE,
    label: 'Gemini 2.5 Flash Lite',
    inputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_FLASH_LITE]?.input ?? '?',
    outputCost: MODEL_PRICING[GEMINI_MODELS.GEMINI_2_5_FLASH_LITE]?.output ?? '?',
  },
    {
    id: GROQ_MODELS.LLAMA_3_1_8B_INSTANT,
    label: 'Llama 3.1 8B Instant',
    inputCost: MODEL_PRICING[GROQ_MODELS.LLAMA_3_1_8B_INSTANT]?.input ?? '?',
    outputCost: MODEL_PRICING[GROQ_MODELS.LLAMA_3_1_8B_INSTANT]?.output ?? '?',
  },
  {
    id: GROQ_MODELS.LLAMA_3_1_70B_VERSATILE,
    label: 'Llama 3.1 70B Versatile',
    inputCost: MODEL_PRICING[GROQ_MODELS.LLAMA_3_1_70B_VERSATILE]?.input ?? '?',
    outputCost: MODEL_PRICING[GROQ_MODELS.LLAMA_3_1_70B_VERSATILE]?.output ?? '?',
  },
  {
    id: GROQ_MODELS.MIXTRAL_8X7B,
    label: 'Mixtral 8x7B',
    inputCost: MODEL_PRICING[GROQ_MODELS.MIXTRAL_8X7B]?.input ?? '?',
    outputCost: MODEL_PRICING[GROQ_MODELS.MIXTRAL_8X7B]?.output ?? '?',
  },
];
...
const AIPanel: React.FC = () => {
  ...
  const savedCustomModel = provider === 'groq' ? (aiSettings.groqCustomModel ?? '') : (aiSettings.geminiCustomModel ?? '');
  const savedModel = provider === 'groq' ? (aiSettings.groqModel ?? DEFAULT_AI_SETTINGS.groqModel ?? '') : (aiSettings.geminiModel ?? DEFAULT_AI_SETTINGS.geminiModel ?? '');
  ...
  useEffect(() => {
    ...
    if (provider === 'groq') {
      if (effectiveModel !== aiSettings.groqModel) {
        saveAiSetting('groqModel', effectiveModel);
      }
    } else {
      if (effectiveModel !== aiSettings.geminiModel) {
        saveAiSetting('geminiModel', effectiveModel);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedModel, customModelStatus, customModelInput]);
  ...
  useEffect(() => {
    ...
    if (provider === 'groq') {
      if (customToSave !== (aiSettings.groqCustomModel ?? '')) {
        saveAiSetting('groqCustomModel', customToSave);
      }
    } else {
      if (customToSave !== (aiSettings.geminiCustomModel ?? '')) {
        saveAiSetting('geminiCustomModel', customToSave);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedModel, customModelStatus, customModelInput]);
  ...
  const validateCustomModel = async () => {
    ...
    try {
      ...
      const testSettings: AISettings = {
        ...
        geminiModel: customModelInput.trim(),
      };
  ...
  return (
    <div className='my-4 w-full space-y-6'>
      ...
      <BoxedList title={_('Provider')} className={disabledSection}>
        ...
        <SettingsRow label={_('Google Gemini')} asLabel>
          <input
            type='radio'
            name='ai-provider'
            className='radio'
            checked={provider === 'google-gemini'}
            onChange={() => setProvider('google-gemini')}
            disabled={!enabled}
          />
        </SettingsRow>
      ...
      <BoxedList
        ...
        description={
          isTauriAppPlatform()
            ...
            : _('Reedy is desktop-only in this beta. Use the DOD desktop app to try it.')
        }
      >
        <SettingsSwitchRow
